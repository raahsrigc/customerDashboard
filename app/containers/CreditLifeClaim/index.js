...
import makeSelectCreditLifeClaim from "./selectors";
...
export function CreditLifeClaim({ sideBarToggle, setSideBarToggle, setSideBarMobileToggle, toggleBtn, setToggleBtn, userData, setUserData }) {

  useInjectReducer({ key: "creditLifeClaim", reducer });
  useInjectSaga({ key: "creditLifeClaim", saga });

  const title = "Credit Life Claim";
  const { TabPane } = Tabs;
  ...
  const cardStats = () => {
    getClaimCardsApi(tokenKey, 10005, toggleBtn)
      .then((res) => {
  ...
  const getClaimHandle = (pageNumber, pageCount, colName, colValue, isSearch, dataStatus) => {
    ...
    getClaimTableApi(tokenKey, pageNumber, pageCount, 10005, colNameArr[colNameArr.length - 1], colValueArr[colValueArr.length - 1], isSearch, toggleBtn)
      .then((res) => {
        res.data = res.data == null ? res.data : JSON.parse(aes256.decrypt(res.data));
        setAllLoading(false)
        if (res.status === true) {
  ...
  const getAllRegisteredClaimsHandle = (pageNumber, pageCount, colName, colValue, isSearch, dataStatus) => {
    ...
    colValue = colValue?.toString();
    setAllLoading(true);
    getAllRegisteredClaimsApi(tokenKey, pageNumber, pageCount, 10005, colNameArr[colNameArr.length - 1], colValueArr[colValueArr.length - 1], isSearch, toggleBtn)
      .then((res) => {
        res.data = res.data == null ? res.data : JSON.parse(aes256.decrypt(res.data));
        setAllLoading(false);
        if (res.status === true) {
  ...
  const getAllApprovedClaimsHandle = (pageNumber, pageCount, colName, colValue, isSearch, dataStatus) => {
    ...
    colValue = colValue?.toString();
    setAllLoading(true);
    getAllApprovedClaimsApi(tokenKey, pageNumber, pageCount, 10005, colNameArr[colNameArr.length - 1], colValueArr[colValueArr.length - 1], isSearch, toggleBtn)
      .then((res) => {
        res.data = res.data == null ? res.data : JSON.parse(aes256.decrypt(res.data));
        setAllLoading(false);
        if (res.status === true) {
  ...
  const getAllSettledClaimsHandle = (pageNumber, pageCount, colName, colValue, isSearch, dataStatus) => {
    ...
    colValue = colValue?.toString();
    setAllLoading(true);
    getAllSettledClaimsApi(tokenKey, pageNumber, pageCount, 10005, colNameArr[colNameArr.length - 1], colValueArr[colValueArr.length - 1], isSearch, toggleBtn)
      .then((res) => {
        res.data = res.data == null ? res.data : JSON.parse(aes256.decrypt(res.data));
        setAllLoading(false);
        if (res.status === true) {
  ...
  const handleCSVDownload = async type => {
    setAllLoading(true)
    if (claimType == "All Claims") {
      getClaimTableApi(tokenKey, 1, 5535, 10005, "", "", false, toggleBtn)
        .then((res) => {
          res.data = res.data == null ? res.data : JSON.parse(aes256.decrypt(res.data));
          setAllLoading(false)
          if (res.status === true) {
      ...
      getAllRegisteredClaimsApi(tokenKey, 1, 5535, 10005, "", "", false, toggleBtn)
        .then((res) => {
          res.data = res.data == null ? res.data : JSON.parse(aes256.decrypt(res.data));
          setAllLoading(false)
          if (res.status === true) {
      ...
      getAllApprovedClaimsApi(tokenKey, 1, 5535, 10005, "", "", false, toggleBtn)
        .then((res) => {
          res.data = res.data == null ? res.data : JSON.parse(aes256.decrypt(res.data));
          setAllLoading(false)
          if (res.status === true) {
      ...
      getAllSettledClaimsApi(tokenKey, 1, 5535, 10005, "", "", false, toggleBtn)
        .then((res) => {
          res.data = res.data == null ? res.data : JSON.parse(aes256.decrypt(res.data));
          setAllLoading(false)
          if (res.status === true) {
...
CreditLifeClaim.propTypes = {
  dispatch: PropTypes.func.isRequired
};

const mapStateToProps = createStructuredSelector({
  creditLifeClaim: makeSelectCreditLifeClaim()
});
